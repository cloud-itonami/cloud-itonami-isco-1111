(ns legislature.actor
  "LegislativeActor — the ISCO-08 1111 legislative office support actor as
  a `langgraph.graph/state-graph` (per ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one office operation request (intake → advise →
  ...
  The unconditional invariant: the LegislativeAdvisor can never directly
  commit a record or dispatch an office action the LegislativeGovernor
  refuses — every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [legislature.advisor :as advisor]
            [legislature.governor :as governor]
            [legislature.store :as store]))

(defn build-graph
  "Build a compiled LegislativeActor graph. `store` implements
  `legislature.store/Store`. `advisor` implements
  `legislature.advisor/Advisor` (defaults to `mock-advisor`).
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:constituent-id (:constituent-id request)
                                    :bill-id (:bill-id request)
                                    :op (:op proposal)
                                    :payload proposal}]
...
(defn run-request!
  "Run one office operation request to completion or interrupt. `thread-id`
